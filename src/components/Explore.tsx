    ...
    <section id="explore" className="explore-section" ref={sectionRef}>
      <div className="explore-sticky">
        ...
        <div className="container explore-inner">
          ...
          <div className={`explore-terminal ${active ? 'live' : ''} ${done ? 'complete' : ''}`}>
            <div className="explore-bar">
              ...
              <div className="explore-progress">
                <div className="explore-progress-fill" style={{ width: `${progress * 100}%` }} />
              </div>
            ...
            <div className="explore-text" aria-live="polite">
              {chars.map((ch, i) => {
                ...
                return (
                  <span
                    key={i}
                    className={`ch ${ch === '\\n' ? 'nl' : ''} ${isNew ? 'fresh' : 'settled'}`}
                  >
                    {ch}
                  </span>
                )
              })}
              <span className={`explore-caret ${!done || active ? 'on' : ''}`} />
            </div>
          </div>

          <div className="explore-meta">
            <span>{Math.round(progress * 100)}%</span>
            <span>{visibleLen} / {FULL_TEXT.length} chars</span>
          </div>
        </div>
      </div>
